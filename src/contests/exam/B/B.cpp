...
private:
  ...
  bool CheckDiagonal1(size_t i, size_t j) const {
    size_t d1 = 0, d2 = 0;
    //std::cerr << "i: " << i << " j: " << j << " d1 check diag1" << std::endl;
    while (d1 <= i && d1 <= j && Table[i][j] == Table[i - d1][j - d1]) {
      ++d1;
    }
    //std::cerr << "i: " << i << " j: " << j << " d2 check diag2" << std::endl;
    while (d2 + i < N && d2 + j < N && Table[i][j] == Table[i + d2][j + d2]) {
      ++d2;
    }
    return d1 + d2 > K;
  }
  ...
  bool CheckDiagonal2(size_t i, size_t j) const {
    size_t d1 = 0, d2 = 0;
    //std::cerr << "i: " << i << " j: " << j << " d1 check diag2" << std::endl;
    while (d1 <= i && d1 + j < N && Table[i][j] == Table[i - d1][j + d1]) {
      ++d1;
    }
    //std::cerr << "i: " << i << " j: " << j << " d2 check diag2" << std::endl;
    while (d2 + i < N && d2 <= j && Table[i][j] == Table[i + d2][j - d2]) {
      ++d2;
    }
    return d1 + d2 > K;
  }
...
// Печатаем поле в поток вывода out
